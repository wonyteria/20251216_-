...
import { LayoutDashboard, Map, Edit3, Shield, Palette, Wallet, Lightbulb, Sparkles, Loader2, Menu } from 'lucide-react';
import { AnyItem, User, Slide, BriefingItem } from '../types';
...
const AdminPage = ({ showToast }: { showToast: (msg: string, type?: 'success' | 'error' | 'info') => void }) => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      setLoading(true);
      try {
        const [usersData, itemsData, slidesData, notisData, briefingData, headersData, commissionData] = await Promise.all([
          database.getUsers(),
          database.getItems(),
          database.getSlides(),
  ...
  const handleSaveUser = async (user: User) => {
    const success = await database.updateUserProfile(user.id, user);
    if (success) {
  ...
  const handleCommissionChange = async (rate: number) => {
    setCommission(rate);
    await database.setCommissionRate(rate);
  };
        ...
        {activeTab === 'permissions' && (
            <div className="bg-white rounded-2xl border shadow-sm overflow-hidden">
                <table className="w-full text-left">
                    <thead><tr className="bg-slate-50 text-xs uppercase text-slate-500"><th className="p-4">유저</th><th className="p-4">이메일</th><th className="p-4">권한</th></tr></thead>
                    <tbody>
                        {users.map(u => (
                ...
                <p className="p-4 text-xs text-slate-400">* 권한 수정은 Supabase Dashboard에서 직접 수정해주세요.</p>
            </div>
        )}
        ...
        {activeTab === 'settlement' && (
            <div className="bg-white rounded-2xl border shadow-sm p-6">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="flex items-center gap-2">수수료율: <input type="number" value={commission} onChange={(e) => handleCommissionChange(Number(e.target.value))} className="w-16 border rounded p-1"/>%</div>
                </div>
