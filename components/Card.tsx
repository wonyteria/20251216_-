...
import { AnyItem, CrewItem, LectureItem, MatchingItem, NetworkingItem, ApplicationStatus } from '../types';
...
const Card: React.FC<CardProps> = ({ item, onClick, isLiked, onToggleLike, isApplied, applicationStatus }) => {
        ...
        return (
            ...
            >
                <div className="relative h-64 overflow-hidden">
                    ...
                    {!isEnded && <LikeButton />}
                    {isApplied && (
                        <div className={`absolute top-3 left-3 z-10 px-2 py-1 rounded-lg text-[10px] font-black border shadow-sm ${getStatusColor(applicationStatus)}`}>
            ...
            return (
                <div onClick={onClick} className="group bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden hover:shadow-lg cursor-pointer relative flex flex-col h-full hover:-translate-y-1 transition-transform duration-300">
                    <div className="relative h-48 overflow-hidden">
                        ...
                        <LikeButton />
                        {isApplied && (
                            <div className={`absolute top-12 left-3 z-10 px-2 py-0.5 rounded text-[9px] font-black border shadow-sm ${getStatusColor(applicationStatus)}`}>
                                {getStatusLabel(applicationStatus)}
                            </div>
                        )}
        ...
        return (
            <div onClick={onClick} className="group bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 overflow-hidden hover:shadow-xl transition-all cursor-pointer relative hover:-translate-y-1">
                <div className="relative h-56">
                    ...
                    <LikeButton />
                    {isApplied && (
                        <div className={`absolute top-3 right-12 z-10 px-2 py-1 rounded-lg text-[10px] font-black border shadow-sm ${getStatusColor(applicationStatus)}`}>
                            {getStatusLabel(applicationStatus)}
                        </div>
                    )}
        ...
        return (
            <div onClick={onClick} className="group cursor-pointer">
                <div className="relative aspect-video rounded-xl overflow-hidden mb-3 shadow-sm group-hover:shadow-md transition-all">
                    ...
                    {!isEnded && <LikeButton />}
                    {isApplied && (
                        <div className={`absolute top-2 right-12 z-10 px-2 py-0.5 rounded text-[10px] font-black border shadow-sm ${getStatusColor(applicationStatus)}`}>
                            {getStatusLabel(applicationStatus)}
                        </div>
                    )}
